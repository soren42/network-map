...
int nm_layout_radial_2d(nm_graph_t *g)
{
    ...
    int *count_at = nm_calloc((size_t)(max_depth + 1), sizeof(int));
    int *placed_at = nm_calloc((size_t)(max_depth + 1), sizeof(int));
    for (int i = 0; i < g->host_count; i++) {
    ...
    for (int i = 0; i < g->host_count; i++) {
        ...
        if (d == 0) {
            ...
            if (g->hosts[i].type == NM_HOST_GATEWAY ||
                g->hosts[i].type == NM_HOST_BOUNDARY) {
                /* Gateways and boundaries in upper half [0, pi] */
                angle = (n > 1) ? (M_PI * k / (n - 1)) : (M_PI / 2);
            } else {
                /* Everything else in lower half [pi, 2*pi] */
...
int nm_layout_3d(nm_graph_t *g)
{
    ...
    for (int i = 0; i < g->host_count; i++) {
        ...
        if (d == 0) {
            ...
            double r = (g->hosts[i].type == NM_HOST_IOT) ?
                       radius * 0.3 : radius;
...
void nm_layout_force_refine(nm_graph_t *g, int iterations)
{
    ...
    double *dx = nm_calloc((size_t)g->host_count, sizeof(double));
    double *dy = nm_calloc((size_t)g->host_count, sizeof(double));
    double *dz = nm_calloc((size_t)g->host_count, sizeof(double));
