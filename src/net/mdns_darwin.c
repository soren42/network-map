#include "config.h"
#if defined(NM_DARWIN) && defined(HAVE_DNS_SD)
...
static nm_graph_t *g_browse_graph = NULL;
                             ...
                             const unsigned char *txt, void *ctx)
{
    ...
    if (getaddrinfo(hosttarget, NULL, &hints, &res) == 0 && res) {
        ...
        int id = nm_graph_find_by_ipv4(g, sin->sin_addr);
        if (id >= 0 && nm_str_empty(g->hosts[id].mdns_name)) {
            nm_strlcpy(g->hosts[id].mdns_name, fullname,
