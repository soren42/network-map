...
int nm_iface_enumerate(nm_graph_t *g)
{
    ...
    if (local_id < 0) {
        ...
        h.hop_distance = 0;
        h.rtt_ms = 0;
        local_id = nm_graph_add_host(g, &h);
    }
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_addr->sa_family == AF_PACKET) {
            ...
            if (sll->sll_halen == 6) {
                ...
                if (!is_zero && !local->has_mac)
                    nm_host_set_mac(local, sll->sll_addr);
            }
        }
#endif
    }
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET) {
            ...
            nm_host_add_ipv4(local, sin->sin_addr);
            nm_host_add_iface(local, ifa->ifa_name);
            iface_count++;
            ...
            nm_host_add_ipv6(local, &sin6->sin6_addr);
            nm_host_add_iface(local, ifa->ifa_name);
            iface_count++;
