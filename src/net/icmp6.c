...
double nm_icmp6_ping(const struct in6_addr *target, int timeout_ms)
{
...
int nm_icmp6_probe(const struct in6_addr *target, int hop_limit,
...
/* Try to extract a MAC address from an EUI-64 link-local IPv6 address.
   Returns 1 if successful, 0 otherwise. */
static int eui64_to_mac(const struct in6_addr *addr, unsigned char mac[6])
{
    const unsigned char *b = addr->s6_addr;
    /* Must be fe80:: link-local */
    if (b[0] != 0xfe || b[1] != 0x80) return 0;
    /* Bytes 2-7 must be zero (no subnet prefix) */
    for (int i = 2; i < 8; i++)
        if (b[i] != 0) return 0;
    /* Check for FF:FE in the middle (EUI-64 marker) */
    if (b[11] != 0xff || b[12] != 0xfe) return 0;
    /* Extract MAC: flip bit 6 of first byte, skip FF:FE */
    mac[0] = b[8] ^ 0x02;
    mac[1] = b[9];
    mac[2] = b[10];
    mac[3] = b[13];
...
int nm_icmp6_discover_neighbors(const char *iface, nm_graph_t *g)
{
    ...
    for (int i = 0; i < 64; i++) {
        ...
        /* Check if this IPv6 addr is already in graph */
        if (nm_graph_find_by_ipv6(g, &from.sin6_addr) >= 0) continue;

        /* Try to match to existing host via EUI-64 MAC extraction */
        unsigned char mac[6] = {0};
        int has_mac = eui64_to_mac(&from.sin6_addr, mac);
        if (has_mac) {
            int mac_host = nm_graph_find_by_mac(g, mac);
            if (mac_host >= 0) {
                /* Merge: add IPv6 address to existing host */
                nm_host_add_ipv6(&g->hosts[mac_host], &from.sin6_addr);
                count++;
                continue;
            }
        }

        /* No MAC match -- create new host */
        nm_host_t h;
        nm_host_init(&h);
