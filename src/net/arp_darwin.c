...
{
    return (mac[0] & 0x01) != 0;  /* low bit of first byte = multicast */
}

static int ip_is_multicast_or_broadcast(struct in_addr addr)
{
    ...
    /* Subnet broadcast: last octet 255 (common /24 subnets) */
...
int nm_arp_read(nm_graph_t *g)
{
    ...
    while (next < lim) {
        ...
        if (mac_is_broadcast(mac)) continue;
        if (mac_is_multicast(mac)) continue;

        struct in_addr ip = sin->sin_addr;
        if (ip_is_multicast_or_broadcast(ip)) continue;

        int host_id = nm_graph_find_by_ipv4(g, ip);

        if (host_id >= 0) {
            if (!g->hosts[host_id].has_mac)
                nm_host_set_mac(&g->hosts[host_id], mac);
        } else {
            nm_host_t h;
            nm_host_init(&h);
            nm_host_set_ipv4_addr(&h, ip);
            nm_host_set_mac(&h, mac);
            h.type = NM_HOST_WORKSTATION;
            if (sdl->sdl_index > 0) {
                char ifname[32];
                if (if_indextoname(sdl->sdl_index, ifname))
                    nm_strlcpy(h.iface_name, ifname, sizeof(h.iface_name));
            }
            host_id = nm_graph_add_host(g, &h);
        }

        if (local_id >= 0 && !nm_graph_has_edge(g, local_id, host_id))
            nm_graph_add_edge(g, local_id, host_id, 1.0, NM_EDGE_LAN);
