...
int nm_route_read(nm_graph_t *g)
{
    ...
    while (next < lim) {
        ...
        int gw_id = nm_graph_find_by_ipv4(g, gw_addr);
        if (gw_id < 0) {
            nm_host_t h;
            nm_host_init(&h);
            nm_host_set_ipv4_addr(&h, gw_addr);
            h.type = NM_HOST_GATEWAY;
            h.hop_distance = 1;
            gw_id = nm_graph_add_host(g, &h);
            count++;
        ...
        if (local_id >= 0 && !nm_graph_has_edge(g, local_id, gw_id)) {
            nm_graph_add_edge(g, local_id, gw_id, 1.0, NM_EDGE_GATEWAY);
        }
    }
