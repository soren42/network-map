...
int nm_dns_reverse_ipv4(struct in_addr addr, char *buf, size_t buflen)
{
...
int nm_dns_reverse_ipv4_ns(struct in_addr ns_addr, struct in_addr target,
...
int nm_dns_resolve_all(nm_graph_t *g, const char *nameserver)
{
    ...
    for (int i = 0; i < g->host_count; i++) {
        ...
        if (!nm_str_empty(h->dns_name)) continue;

        char name[NM_HOSTNAME_LEN];
        int rc;

        if (use_custom)
            rc = nm_dns_reverse_ipv4_ns(ns_addr, h->ipv4, name, sizeof(name));
        else
            rc = nm_dns_reverse_ipv4(h->ipv4, name, sizeof(name));

        if (rc == 0) {
            nm_strlcpy(h->dns_name, name, sizeof(h->dns_name));
            LOG_DEBUG("DNS: %s -> %s", nm_host_ipv4_str(h), name);
            count++;
        }
    }
    return count;
